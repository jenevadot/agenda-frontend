import { X, User, Phone, Mail, Scissors, Calendar } from 'lucide-react';
...
function getEstadoInfo(estado: string): { label: string; className: string } {
  switch (estado) {
    ...
    case 'no_asistio':
      return { label: 'No asistio', className: 'bg-danger-light text-danger' };
    default:
 ...
 */
export function ModalDetalleCitaAdmin({ cita, onCerrar }: Props) {
      ...
      <div className="relative min-h-screen flex items-center justify-center p-4">
        <div className="relative bg-white rounded-2xl shadow-xl max-w-md w-full animate-slide-up">
          ...
          <div className="p-4 border-t border-gray-100">
            <Button variant="secondary" onClick={onCerrar} className="w-full">
              Cerrar
            </Button>
          </div>
        </div>
